...
- production credential/key store; the current `moxi-vault` MVP models
  ...
  secret injection evidence decisions, trust-root storage decisions, and a
  fail-closed production readiness decision, but not live KMS/HSM calls, real
...
13. Build `moxi-vault` as the first P0 production-hardening control plane for
   ...
   evidence, adapter verification decisions, rotation enforcement decisions,
   secret injection evidence decisions, compliance export delivery decisions,
